...
export default function Home() {
  ...
  return (
    ...
    >
      {/* Subtle hero grid */}
      <div className="pointer-events-none absolute inset-x-0 top-0 h-full opacity-[0.08]">
        <div className="h-full bg-[linear-gradient(to_right,rgba(15,23,42,0.5)_1px,transparent_1px),linear-gradient(to_bottom,rgba(15,23,42,0.35)_1px,transparent_1px)] bg-[size:72px_72px]" />
      </div>
      ...
      <main className="relative z-10 w-full max-w-6xl px-6 py-8 mx-auto lg:px-10 lg:py-12">
        ...
        <section className="mt-12 space-y-4">
          <div className="flex items-center justify-between gap-2">
            <div>
              ...
              <h2 className="text-sm font-semibold text-zinc-900">
                Curated Discord perks from trusted sellers
              </h2>
            </div>
          </div>
          ...
          {productsLoading ? (
            ...
            <p className="text-xs text-zinc-500">
              No products yet. Once sellers start listing Nitro, boosts and OG
              handles, they&apos;ll appear here.
            </p>
          ) : (
            <div className="grid gap-4 md:grid-cols-3">
              {products.map((p) => (
                <Link
                  key={p.id}
                  href={`/products/${p.id}`}
                  className="group rounded-2xl border border-zinc-200 bg-white/90 p-4 text-xs text-zinc-700 shadow-[0_18px_45px_rgba(15,23,42,0.06)] backdrop-blur transition hover:-translate-y-1 hover:border-indigo-200/80 hover:shadow-[0_22px_55px_rgba(15,23,42,0.12)]"
                >
